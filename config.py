...
def _state_file_candidates():
    return [PROJECT_ROOT / STATE_FILENAME, Path.home() / ("." + STATE_FILENAME)]


def _legacy_state_candidates():
    return [PROJECT_ROOT / "curiosity_viewer_state.json", Path.home() / ".curiosity_sol_viewer_state.json"]


def load_app_state() -> dict[str, Any]:
    current = _state_file_candidates()
    for path in current + _legacy_state_candidates():
        try:
