...
from planetary_studio.annotations.image_annotations import AnnotationWindowMixin, composite_image
from planetary_studio.ui.image_filters import ImageFiltersMixin
from planetary_studio.annotations.sol_annotations import SolAnnotationsMixin, AnnotationThumbnailDelegate
from planetary_studio.processing.image_smoothing import SmoothingDialog, smooth_image
from planetary_studio.processing.image_adjustments import AdjustmentDialog, apply_adjustments
from planetary_studio.ui.inline_adjustments import InlineAdjustmentsMixin
from planetary_studio.ui.adjustment_profiles import AdjustmentProfilesMixin, load_profiles
from planetary_studio.ui.toolbar_appearance import ToolbarAppearanceMixin
...
from planetary_studio.config import APP_NAME, APP_VERSION, APP_AUTHOR, PROJECT_ROOT, load_app_state, save_app_state
from planetary_studio.collections.catalog import list_images
from planetary_studio.sources import get_source, SOURCES
from planetary_studio.sources.base import ImageSource, MetadataProvider
...
class MainWindow(AdjustmentProfilesMixin, ToolbarAppearanceMixin, InlineAdjustmentsMixin, SolAnnotationsMixin, ImageFiltersMixin, AnnotationWindowMixin, QMainWindow):
    ...
    def _export_auto_enhance_stages(self):
        ...
        if not folder:
            return
        try:
            from planetary_studio.processing.image_adjustments import save_auto_enhance_stages
    ...
    def _restore_removed_images(self):
        ...
        if cleanup is not None and cleanup.worker is not None:
            ...
            return
        from planetary_studio.collections.exact_duplicates import REGISTRY,read_registry
        from planetary_studio.collections.restore_removed import validated_paths
        try:
    ...
    def _open_forensic_texture(self):
        if self.original_image is None or self.current_path is None:
            ...
            return
        try:
            from planetary_studio.ui.forensic_texture_ui import ForensicTextureDock
    ...
    def _open_morphological(self):
        if self.original_image is None:
            ...
            return
        try:
            from planetary_studio.ui.morphological_region_ui import marked_regions, MorphologicalRegionDialog
        except ImportError as exc:
            QMessageBox.warning(self, "Dependências da análise", f"Instale as dependências de requirements.txt.\n\n{exc}")
            return
    ...
    def _open_magic_wand(self):
        if self.original_image is None or self._annotation_document is None:
            ...
            return
        try:
            from planetary_studio.ui.magic_wand_ui import MagicWandDialog
        except ImportError as exc:
