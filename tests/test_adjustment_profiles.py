...
from planetary_studio.ui.adjustment_profiles import PROFILE_KEYS,load_profiles
from planetary_studio.config import load_app_state
from planetary_studio.annotations.image_annotations import empty_state,AnnotationDocument
...
class ProfileIntegrationTests(unittest.TestCase):
    ...
    def test_save_pending_apply_all_controls_and_single_undo_on_other_image(self):
        ...
        from planetary_studio.app import pil_to_qimage
        from planetary_studio.processing.image_adjustments import apply_adjustments
    ...
    def test_slots_persist_to_disk_and_remain_global_across_missions(self):
        ...
        with patch.object(MainWindow,'_save_state',SAVE_STATE), \
             patch('planetary_studio.config._state_file_candidates',return_value=[self.root/'session.json']), \
             patch('planetary_studio.config._legacy_state_candidates',return_value=[]):
            w=self.window()
    ...
    def test_save_error_is_not_reported_as_saved_and_empty_slots_are_disabled(self):
        ...
        with patch.object(w,'_save_state',return_value=False),patch('planetary_studio.ui.adjustment_profiles.QMessageBox.warning') as warning:
            w._profile_buttons[0][1].click()
