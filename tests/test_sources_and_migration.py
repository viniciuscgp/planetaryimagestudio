...
import planetary_studio.config as config
from planetary_studio.app import MainWindow
from planetary_studio.sources import get_source
from planetary_studio.sources.curiosity import DownloaderWorker


class MigrationTests(unittest.TestCase):
    def test_project_root_is_independent_of_package_directory(self):
        self.assertEqual(config.PROJECT_ROOT, Path(__file__).resolve().parents[1])
        self.assertEqual(config.DATA_ROOT, config.PROJECT_ROOT / "data")

    def test_root_session_moves_to_data_and_preserves_original(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            old = root / config.STATE_FILENAME
            state = {"source": "local", "root": "my-collection", "selected_image": "moon.png"}
            old.write_text(json.dumps(state), encoding="utf-8")
            original = old.read_bytes()
            with patch.object(config, "PROJECT_ROOT", root), \
                 patch.object(config, "DATA_ROOT", root / "data"), \
                 patch.object(config.Path, "home", return_value=root / "home"):
                self.assertEqual(config.load_app_state(), state)
                migrated = root / "data" / config.STATE_FILENAME
                self.assertEqual(json.loads(migrated.read_text(encoding="utf-8")), state)
                self.assertEqual(old.read_bytes(), original)
                updated = dict(state, selected_image="mars.png")
                self.assertEqual(config.save_app_state(updated), migrated)
                self.assertEqual(config.load_app_state(), updated)
...
class SourceTests(unittest.TestCase):
    ...
    def test_source_switch_restores_collection_and_selected_image(self):
        ...
        with patch("planetary_studio.app.QFileDialog.getExistingDirectory", return_value=str(moon)):
            window._choose_source("local")
        self.assertEqual(window.source.id, "local")
        self.assertEqual(window.current_path, moon / "moon.png")
        with patch("planetary_studio.app.QFileDialog.getExistingDirectory", return_value=str(self.root)):
