...
class SessionRestoreTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for target,value in [('planetary_studio.config._state_file_candidates',[Path(self.temp.name)/'session.json']),('planetary_studio.config._legacy_state_candidates',[])]:
            p=patch(target,return_value=value);p.start();self.addCleanup(p.stop)
        p=patch('planetary_studio.app.QTimer.singleShot');p.start();self.addCleanup(p.stop)
