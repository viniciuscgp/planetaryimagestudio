...
from planetary_studio.processing.image_adjustments import (auto_enhance_mars_image,detect_useful_area,gray_world_balance,
...
from planetary_studio.annotations.image_annotations import empty_state,validate_state,AnnotationDocument
from planetary_studio.ui.image_filters import is_edited
...
class AutoEnhanceTests(unittest.TestCase):
    ...
    def test_stage_exports_and_auto_starts_from_clean_original(self):
        ...
        with patch('planetary_studio.processing.image_adjustments.auto_enhance_mars_image',wraps=auto_enhance_mars_image) as enhance:
...
class AutoEnhanceIntegrationTests(unittest.TestCase):
    ...
    def test_export_uses_original_even_with_visible_annotations(self):
        ...
        with tempfile.TemporaryDirectory() as folder, \
             patch('planetary_studio.app.QFileDialog.getExistingDirectory',return_value=folder), \
             patch('planetary_studio.app.QMessageBox.information'), \
             patch('planetary_studio.processing.image_adjustments.save_auto_enhance_stages',return_value=Path(folder)) as export:
