...
from image_annotations import AnnotationDocument, image_transform
...
class AnnotationTests(unittest.TestCase):
    ...
    def test_sidecar_roundtrip_undo_redo_and_original_unchanged(self):
        ...
        with patch("image_annotations.QInputDialog.getText", return_value=("Alvo α", True)):
    ...
    def test_rotation_keeps_drawings_registered_to_original_pixels(self):
        ...
        window = self.window()
        with patch("image_annotations.QColorDialog.getColor", return_value=QColor("blue")):
            window._choose_stroke_color()
    ...
    def test_cancel_text_and_reset_adjustments(self):
        self.image(10, "first.png")
        window = self.window()
        with patch("image_annotations.QInputDialog.getText", return_value=("", False)):
    ...
    def test_atomic_save_failure_preserves_previous_file_and_unsaved_edits(self):
        ...
        with patch("image_annotations.Path.replace", side_effect=OSError("Disk full")), patch("image_annotations.QMessageBox.warning"):
            self.draw(window, "circle")
    ...
    def test_close_can_be_cancelled_when_saving_fails(self):
        self.image(10, "first.png")
        window = self.window()
        with patch("image_annotations.AnnotationDocument.save", side_effect=OSError("Disk full")), patch(
            "image_annotations.QMessageBox.warning", return_value=QMessageBox.StandardButton.Cancel
        ):
            self.draw(window, "pencil")
